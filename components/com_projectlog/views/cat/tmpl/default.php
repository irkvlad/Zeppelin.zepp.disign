        ...
        <form name="adminForm" method="get" action="index.php">
            <table class="ptable" width="100%" cellpadding="5" cellspacing="1">
				...
				foreach ($this->projects as $p) :
                    ...
                    $last_fild_on        = '</td><td align="center"><a target="_blank" href="' . $calendar_link . '" class="red">[' . JText::_('CALENDAR') . ']</a></td> ';
                    if ($cat_id == 0){
                        switch ($p->category){
                            case 5:
                                $last_fild_on = '</td><td align="center">' . JText::_('Дизайн') . ' <br> ';
                                if($p->designer)  $last_fild_on .= projectlogHTML::getusername($p->designer); else $last_fild_on .= "<b>Требуется дизайнер</b>";
                                $last_fild_on .= '</td> ';
                            break;
                            case 6:
                                $last_fild_on = '</td>
                    ...
                    echo '</td><td align="center">' . $p->release_id . '<br />';
                    ...
                    echo '</td><td><a href="' . $proj_link . '">' . $p->title . '</a>';
                    echo '</td><td>' . projectlogHTML::getusername($p->manager)
                        . '</td><td align="center">' . projectlogHTML::getusername($p->technicians)
                        . '</td><td align="center">' . projectlogHTML::getContactName($p->brigadir)
                        . '</td>';
                    echo $last_fild_on;
