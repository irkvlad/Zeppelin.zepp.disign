<?php
/**
 * 		Вспомогательные функции для УправленияПроектами
 *
...
{
	/**
	 *  Получить группу пользователя в УправленииПроектами
	 * @return mixed
	 */
	public function getUserGrup(){
		...
		$query = 'SELECT group_id FROM #__projectlog_groups_mid WHERE user_id = ' . $user->id;
		$db->setQuery($query);
		$ob = $db->loadResult();
		return  $ob ;
	}
    ...
    /**
     * Записать данные в базу для отправки сообщения через zeppMessenders
     *
     * @param $from_user_id - от кого
     * @param $text - текст
     * @param $to_user_id - кому
     * @param $tema - тема сообщения
     * @param $project_id - проект
     * @return mixed - результат запроса в базу
     */
	function savePochta($from_user_id,$text,$to_user_id,$tema,$project_id){
        $db = JFactory::getDBO();
	...
	function buildThinkeryFooter($accent_color){
		$html = '';
		$html .= '<div class="rc_project_footer"></div>';
		return $html;
	}
    ...
    /**
     * Получить номер меню(ItemId) данного вида
     *
     * @param $view
     * @return mixed
     */
    ...
    function getProjectNumber($project_id){
		$database = JFactory::getDBO();
		$database->setQuery("SELECT release_id FROM #__projectlog_projects WHERE id = " . $project_id);
	...
	function getProject($project_id){
		$database = JFactory::getDBO();
		$database->setQuery("SELECT * FROM #__projectlog_projects WHERE id = " . $project_id);

		return $database->loadObject();
	}

    /**
     * Имя пользователя по EMAIL
     * @param $user_id
     * @return string
     */
    ...
    /**
     * Настройка отправки почты 1- отправлять 0-не отправлять
     * @param $user_id чья настройка.
     * @return mixed
     */
    ...
    /**
     * Детали пользователя по данным Контактов
     *
     * @param $user_id
     * @return mixed
     */
    ...
    /**
     * Почтовый адрес по ID пользователя
     * @param $user_id
     * @return mixed
     */
		...
		switch ($type){
			case 'brak':
				if ($type and $proect->category == 12 and $proect->location_spec <> ''){
					...
					JUtility::sendMail(SITE_EMAIL, SITE_NAME, $z_email, $subject, $body, $mode, $cc=null, $bcc=null, $attachment, $from_email, $from_name);
					JUtility::sendMail(SITE_EMAIL, SITE_NAME, $admin_email, $subject, $body, $mode, $cc, $bcc, $attachment, $from_email, $from_name);
                    JUtility::sendMail(SITE_EMAIL, SITE_NAME, $a3_email, $subject, $body, $mode, $cc, $bcc, $attachment, $from_email, $from_name);
			...
			case 'manager':
				...
				if (projectlogHTML::getUserPChekc($onLoadM) == 1)
					JUtility::sendMail(SITE_EMAIL, SITE_NAME, projectlogHTML::userEmail($onLoadM), $subject, $body, $mode, $cc, $bcc, $attachment, $from_email, $from_name);

				// Для ZeppProjekt
				$db    = JFactory::getDBO();
				$query = "INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) 
							VALUES ( " . $user->id . " ,  '" . $body . "' , " . $proect->manager . " , 4 , " . $proect->id . " ) ;";
				$db->setQuery($query);
				if (!$db->query()){
				...
				$db    = JFactory::getDBO();
				$query = "INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) 
							VALUES ( " . $user->id . " ,  '" . $body . "' , " . $user->id . " , 4 , " . $proect->id . " ) ;";
				$db->setQuery($query);
				if (!$db->query()){
					$this->setError($db->getErrorMsg());
				}
				...
				if ($proect->category == 7 or $proect->category == 8 or $proect->category == 13){
					...
					$db    = JFactory::getDBO();
					$query = "INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) 
						VALUES ( " . $user->id . " ,  '" . $body . "' , " . $proect->technicians . " , 4 , " . $proect->id . " ) ;";
					$db->setQuery($query);
					if (!$db->query()){
						$this->setError($db->getErrorMsg());
					}
			...
			case 'release_date':
				...
				$body = '<p>' . $add_type . '</p>'
					...
					.'<p> Предыдущая дата реализации:&nbsp;' . JHTML::_('date', $onLoadM, JText::_('%d.%m.%Y')) . '. <br />'
					.'Перенесена на:&nbsp;' . JHTML::_('date', $proect->release_date, JText::_('%d.%m.%Y')) . ' . </p>'
		            .'<p>  <i> ' . $proect->location_spec . '</i><br />'
		            .'<span style="font-size: 10px; color: #999;">IP: ' . $ipaddress . '</span></p>'
				;
				// JUtility::sendMail(SITE_EMAIL, SITE_NAME, 'vl@zepp.ru', $subject, $body, $mode, $cc, $bcc, $attachment, $from_email, $from_name);
				if (projectlogHTML::getUserPChekc($proect->manager) == 1)
					JUtility::sendMail(SITE_EMAIL, SITE_NAME, $m_email, $subject, $body, $mode, $cc=null, $bcc=null, $attachment, $from_email, $from_name);
				...
				if ($proect->category == 7 or $proect->category == 8 or $proect->category == 13){
					...
					if (projectlogHTML::getUserPChekc(97) == 1)
						JUtility::sendMail(SITE_EMAIL, SITE_NAME, $a2_email, $subject, $body, $mode, $cc=null, $bcc=null, $attachment, $from_email, $from_name);
	...
	function notifyDoc($type, $user, $project_id, $faile){	
	    ...
	    if ($type){
		    switch ($type){
				...
				$text = '<strong>'
					. JText::_('PROJECT MANAGER')
					. ' : </strong>'
					. projectlogHTML::getusername($faile[0]->manager)
					. '<br />
		                 <strong>'
					. JText::_('PROJECT DELETED')
					. '</strong> '
					. $user->name
					. ' &nbsp;&nbsp;&nbsp;&nbsp;('
					. $date
				...
				$text = '<strong>'
					...
					. ' : </strong><p>в полях:<br /> '
					. $faile
					. ' были изменены значение</p> <strong>'
					. JText::_('Значения изменил:')
					. '</strong> '
					. $user->name
					. ' &nbsp;&nbsp;&nbsp;&nbsp;('
					. $date
		    ...
		    $body = '<p>'. $add_type. '</p>'
                . '<p><strong> <a href="'. $link_project . '"> ' . JText::_('PROJECT') . '</a>:&nbsp;&nbsp;&nbsp;</strong>'. $proect->title. '<br />'
			    . '<strong>' . JText::_('RELEASE NUM')	. ':&nbsp;&nbsp;&nbsp;</strong> ' . $proect->release_id . '</p>' . '<br />'
			    . $text . '<br />'
...
//$this->setError("In pochta2");
		    ...
		    // Менеджеру проекта
		    if ($user_email <> $m_email and $m_email <> '' ){
                if (projectlogHTML::getUserPChekc($proect->manager) == 1) JUtility::sendMail(SITE_EMAIL, SITE_NAME, $m_email, $subject, $body, $mode, $cc, $bcc, $attachment, $from_email, $from_name);
		    ...
		    if ($proect->category == 7 or $proect->category == 8 or $proect->category == 12 or $proect->category == 13){
			    $body = $body . '<br />Сообщения о комментарии отправленны на почтовые адреса:<br />' . $toto_email . ' Вам : ' . $user_email;

			    if (projectlogHTML::getUserPChekc($user->id) == 1) JUtility::sendMail(SITE_EMAIL, SITE_NAME, $user_email, $subject, $body, $mode, $cc, $bcc, $attachment, $from_email, $from_name);
			    // Для ZeppProjekt
		    ...
		    if ( $proect->category == 5 ){
                $query = 'SELECT loggedby FROM #__projectlog_logs WHERE project_id = ' . $project_id.' GROUP BY loggedby ';
                $db->setQuery($query);
                $loggedbys = $db->loadObjectlist();
                foreach ($loggedbys as $loggedby){
                    $loggedby= $loggedby->loggedby;
                    if( $user->id <> $loggedby) {
                        $to_email = projectlogHTML::useremail($loggedby);
                        $to_name  = projectlogHTML::getusername($loggedby);
                        if (projectlogHTML::getUserPChekc($loggedby) == 1) JUtility::sendMail(SITE_EMAIL, SITE_NAME, $to_email, $to_name, $body, "HTML");
                        $toto_email = $toto_email . ' Менеджер:' . $m_name . '- ' . $m_email . '<br />';

                        // Для ZeppProjekt
                        $query = "INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) VALUES ( " . $user->id . " ,  '" . $body . "' , " . $loggedby . " , " . $pochta_chek . " , " . $idd . " ) ;";
                        $db->setQuery($query);
                        if (!$db->query()) $this->setError($db->getErrorMsg());
                    }
                }
 ...
 */
	function notifyUsers($project_id, $mov, $msg){
        ...
        if ($project_id){
            ...
            $project_date = new DateTime($proect->disign_date);

            if ($proect->manager){
                $managerdetails = projectlogHTML::userDetails($proect->manager);
                if ($managerdetails){
                    $m_email   = $managerdetails->email_to;
                    $m_telefon = $managerdetails->telephone;
                    $m_name = $managerdetails->name;
                }
            }
            ...
            switch ($mov){
                // Новый проект в работу, отправляются сообщения начальнику и секретрю производства а так же технологу
                case '7':
                    $subject = "Новый проект OT " . projectlogHTML::getusername($proect->manager);

                    $body = '  <strong>В работу, ' . $date . ', поступил новый проект: </strong>'
                        .'<br><strong>Контрагент: </strong>' . $proect->title
                        .'<br /><strong>Номер заказа: </strong>' . $proect->release_id
                        .'<br /><strong>Менеджер: </strong>' . projectlogHTML::getusername($proect->manager) . '<strong>  Телефоны: </strong>' . $m_telefon
                    ...
                    if (projectlogHTML::getUserPChekc($TEH_DOCER) == 1)
                        JUtility::sendMail(SITE_EMAIL, SITE_NAME, $teh_doker_email, $subject, $body, $mode, $cc, $bcc, $attachment);

                    if (projectlogHTML::getUserPChekc($TEH_ADMIN) == 1)
                        JUtility::sendMail(SITE_EMAIL, SITE_NAME, $teh_admin_email, $subject, $body, $mode, $cc, $bcc, $attachment);

                    if ($proect->technicians){
                        $temail = projectlogHTML::userEmail($proect->technicians);
                        if (projectlogHTML::getUserPChekc($proect->technicians) == 1)
                            JUtility::sendMail(SITE_EMAIL, SITE_NAME, $temail, $subject, $body, $mode, $cc, $bcc, $attachment);
                    }
                    $to_email[] = 'irkvlad@hotmail.com';

                    //JUtility::sendMail(SITE_EMAIL, SITE_NAME, $to_email, "Админу", $body, 1);

                    // Для ZeppProjekt
                    $query = "INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) VALUES ( " . $proect->manager . " ,  '" . $body . "' , 97 , 3 , " . $project_id . " ) ;";
                    $db->setQuery($query);
                    if (!$db->query()) $this->setError($db->getErrorMsg() . $moveDate . $category + 1);
                ...
                case '8':{
                    ...
                    if ($proect->manager){
                        $managerdetails = projectlogHTML::userDetails($proect->manager);
                        if ($managerdetails){
                            $teh_doker_email = $managerdetails->email_to;
                        }
                    }else{
                        $teh_doker_email = '';
                    }
                ...
                case '12':{
                    ...
                    $db    = JFactory::getDBO();
                    $query = "INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) 
                            VALUES ( 97 ,  '" . $body . "' , " . $proect->manager . " , 3 , " . $project_id . " ) ;";
                    $db->setQuery($query);
                    if (!$db->query()){
                        $this->setError($db->getErrorMsg() . $moveDate . $category + 1);
                    }
                ...
                case '9':{
                    ...
                    $body = $date . ' <strong>был отправлен на гарантию проект созданный ' . projectlogHTML::getusername($proect->created_by) . ' : </strong><br>
                            <strong>Контрагент: </strong>' . $proect->title .
                        '<br /><strong>Номер заказа: </strong>' . $proect->release_id .
                        '<br /><strong>Текущий менеджер: </strong>' . projectlogHTML::getusername($proect->manager) . '<strong>  Телефоны: </strong>' . $m_telefon .
                        '<br /><strong>Технолог: </strong>' . projectlogHTML::getusername($proect->technicians) .
                    ...
                    if ($proect->manager){
                        ...
                        if ($managerdetails){
                            ...
                            if (projectlogHTML::getUserPChekc($proect->manager) == 1)
                                JUtility::sendMail(SITE_EMAIL, SITE_NAME, $teh_doker_email, $subject, $body . $bodyM, $mode, $cc, $bcc, $attachment);
                        }
                    }
                ...
                case '13':{
                    ...
                    JUtility::sendMail(SITE_EMAIL, SITE_NAME, 'black@zepp.ru', $subject, $body, $mode, $cc, $bcc, $attachment);
                    JUtility::sendMail(SITE_EMAIL, SITE_NAME, 'vl@zepp.ru', $subject, $body, $mode, $cc, $bcc, $attachment);
	...
	function kalendarik($w_k, $shot_title, $release_date, $job_id, $release_id, $technicians, $projecttype, $workorder_id, $path, $w, $h, $podrydchik)
{
	...
	function kal_info($podrydchik, $task_id, $client, $manager, $chief, $technicians, $id, $doc_path, $description, $location_gen){
        ...
        echo '<span class="content_header">' . JText::_('CAST PROJECT') . ':</span>'
            .'<span class="red2">'
            . ($task_id) ? $task_id : '&nbsp;'
            .'</span><br />'
        ...
        echo '<br /><span class="content_header">' . JText::_('DISAINER') . ':</span>';
            ...
            foreach ($docs as $d) {
                ...
                echo '<div class="doc_item"> > '
                    . '<a href="' . $doc_path . $id . '/' . $d->path . '"
                         ...
                         target="_blank"
                         class="hasTip"
                         title="' . JText::_('DOCUMENT') . ' :: ' . JText::_('SUBMITTED BY') . ': '
                    . projectlogHTML::getusername($d->submittedby) . ' <br /> '
                    . JText::_('FILE') . ': ' . $d->path . ' <br /> '
                    . JText::_('SUBMITTED DATE') . ': ' . $d->date . '" >'
                    . $d->name . '</a>'
                    . '</div>';
            }
            foreach ($logo as $l):
                echo '<div class="doc_item">'
                    ...
                    class="hasTip"
                    title="' . JText::_('DOCUMENT') . ' :: ' . JText::_('SUBMITTED BY') . ': '
                    . projectlogHTML::getusername($l->submittedby) . ' <br /> '
                    . JText::_('FILE') . ': ' . $l->path . ' <br /> '
                    . JText::_('SUBMITTED DATE') . ': '
                    . $l->date . '" > ' . $l->path . '</a>';
        ...
        echo '<div class="right_details">'
            ...
            .'<div style="margin: 10px;background-color:#fff;padding:5px;" class="right_details"><div class="content_header2">'
            . JText::_('DESCRIPTION')
            . ':</div><div style="color:#000;background-color:#fff;" >';
        if ($description) echo $description;
        echo '<span class="content_header">' . JText::_('GEN LOC') . ':</span><br />';
        if ($location_gen) echo $location_gen;
        echo '</div></div><br /></div>';
    }
